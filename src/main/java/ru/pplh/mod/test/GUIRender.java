...
import static ru.pplh.mod.PepeLandHelper.playerInPPL;
...
public class GUIRender implements GuiRenderEvents {
    ...
    public void onRender(GuiGraphicsExtractor guiGraphics, float partialTick) {
        ...
        if(InteractiveManager.currentInteractive != null && playerInPPL()){
            ...
            guiGraphics.text(AlinLib.MINECRAFT.font, String.format("Интерактив: %s", inter.id), x, y, -1);
            y+=AlinLib.MINECRAFT.font.lineHeight+3;
            guiGraphics.text(AlinLib.MINECRAFT.font, String.format("Статус интерактива: %s", inter.status), x, y, -1);
            y+=AlinLib.MINECRAFT.font.lineHeight+3;
            guiGraphics.text(AlinLib.MINECRAFT.font, "Действия:", x, y, -1);
            y+=AlinLib.MINECRAFT.font.lineHeight+3;
            for(Action action : inter.actions){
                ...
                for(FormattedCharSequence text : AlinLib.MINECRAFT.font.split(Component.literal(msg), (int) (guiGraphics.guiWidth()*0.33))) {
                    guiGraphics.text(AlinLib.MINECRAFT.font, text, x, y, -1);
                    y+=AlinLib.MINECRAFT.font.lineHeight+3;
                }
            }
        }
