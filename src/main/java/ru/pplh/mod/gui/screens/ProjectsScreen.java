...
public class ProjectsScreen extends AbstractPPLScreen {
    ...
    public void initCategory() {
        ...
        addRenderableWidget(new EditBoxBuilder(Component.translatable("pplhelper.news.search"), (s) -> query = s).setValue(query).setPosition(getX(), y[0]).setWidth(getContentWidth() - 25).build());
        ...
        addRenderableWidget(new SelectorBuilder(Component.translatable("pplhelper.project.category"), (s) -> {
            ...
            search();
        }).setList(PepeLandHelper.pc).setValue(category).setPosition(getX() + 5 + searchSize, y[0]).setWidth(searchSize).build());
        y[0] += 25;
        pageControlWidget =  addRenderableWidget(new PageControlWidget(getX(), y[0], getContentWidth(), 20, currentPosition, 1, (widget) -> {
