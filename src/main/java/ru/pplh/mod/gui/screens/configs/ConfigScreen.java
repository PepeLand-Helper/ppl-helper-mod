...
public class ConfigScreen {
    ...
    public Screen build(Screen parent) {
        ...
        if (PepeLandHelper.isTestSubject()) {
            builder.addPanelWidget(new ButtonBuilder(Component.literal("Crash me!")).setIcon(DONT).setOnPress((s) -> {
                try {
                    throw new RuntimeException("Эта карусель крутится и никогда не остановится!");
                } catch (Exception ex) {
                    AlinLib.MINECRAFT.gui.setScreen(new ErrorScreen(ex, AlinLib.MINECRAFT.gui.screen()));
                }
            })).addPanelWidget(new ButtonBuilder(Component.literal("Crash me! (без exception)")).setIcon(DONT).setOnPress((s) -> {
                try {
                    throw new RuntimeException("Эта карусель крутится и никогда не остановится!");
                } catch (Exception ex) {
                    AlinLib.MINECRAFT.gui.setScreen(new ErrorScreen(AlinLib.MINECRAFT.gui.screen()));
                }
            }));
        }
        ...
        builder.addWidget(new HorizontalRuleBuilder(Component.literal("Locator Bar")))
                .addWidget(new ButtonBooleanBuilder(Component.translatable("pplhelper.configs.locator_bar.trade"), true).setConfig(PepeLandHelper.config, "LOCATOR_BAR.TRADE"));
        if(FabricLoader.getInstance().isModLoaded("pochatok")){
            builder.addWidget(new HorizontalRuleBuilder(Component.literal("Початок")))
                    .addWidget(new ButtonBooleanBuilder(Component.translatable("pplhelper.configs.pochatok.use_registry"), true).setConfig(PepeLandHelper.config, "POCHATOK.USE_REGISTRY"));
        }
