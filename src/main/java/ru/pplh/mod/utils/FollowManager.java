    ...
    public record Coordinates(String name, String level, TabHelper.Worlds world, int[] coordinates) {
        ...
        public Vec3 pos(){
            if(coordinates.length == 2){
                return new Vec3(getCord(coordinates[0]), AlinLib.MINECRAFT.player.position().y, getCord(coordinates[1]));
            } else return new Vec3(getCord(coordinates[0]), getCord(coordinates[1]), getCord(coordinates[2]));
        }
